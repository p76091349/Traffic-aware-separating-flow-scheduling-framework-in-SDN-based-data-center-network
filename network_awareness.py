...
class NetworkAwareness(app_manager.RyuApp):
	...
	def __init__(self, *args, **kwargs):
		...
		self.access_table = self.create_access_table(4)                # {(sw,port):(ip, mac),}
	...
	def get_topology(self, ev):
		...
		if present_time - self.start_time < self.initiation_delay:
			return
                #print("awareness get network topology")

		#self.logger.info("[GET NETWORK TOPOLOGY]")
		switch_list = get_switch(self.topology_api_app, None)
                sw_dpid=[s.dp.id for s in switch_list]
		...
		for src in _graph.nodes():
			...
			for dst in _graph.nodes():
				...
				paths[src][dst] = self.k_shortest_paths(_graph, src, dst, weight=weight, k=k)
                                #print('path',src,dst,paths[src][dst])
		return paths
	...
	def show_topology(self):
		...
		#nx.draw(self.graph)
                #img_path='/home/result/%d.png' % int(time.time())
