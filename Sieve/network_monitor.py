...
class NetworkMonitor(app_manager.RyuApp):
	...
	def _monitor(self):
		...
		"""
		    ...
		    hub.sleep(setting.MONITOR_PERIOD)	#init is 2
	...
	def _flow_stats_reply_handler(self, ev):
		...
		if flow_num > 0:
			...
			for stat in sorted([flow for flow in body if ((flow.priority not in [0, 1000, 10, 65535]) and (flow.instructions[0].actions[0].port == self.sw_out_inf[dpid]) and (flow.byte_count > 50000) and flow.match.get('tcp_src'))],
							   key=lambda flow: (flow.priority, flow.match.get('ipv4_src'), flow.match.get('ipv4_dst'))):
				key = (dpid, stat.instructions[0].actions[0].port)
				...
				paths.append([stat.match.get('eth_type'), stat.match.get('in_port'), stat.match.get('ipv4_src'), stat.match.get('ipv4_dst'), stat.match.get('tcp_src'), stat.match.get('tcp_dst'), stat.priority])
                                print([stat.match.get('eth_type'), stat.match.get('in_port'), stat.match.get('ipv4_src'), stat.match.get('ipv4_dst'), stat.match.get('tcp_src'), stat.match.get('tcp_dst'), stat.priority])
                                print('packet count',flow.packet_count)
 
                            
                                print('flow size',flow.byte_count)
                        #print('elephant flow',paths)
                        
			if key:
				flow_port[key] = paths
				port_flow_num = len(flow_port[key])
				
				load_thre = 0.25
				
				if dpid > 3000:
					capacity = self.port_capacity[3][stat.instructions[0].actions[0].port]
				...
				if self.redir_flow_num > 0 and load_current_port >= load_thre:
					for i in range(0,self.redir_flow_num):
						self.get_path_by_fqouta(dpid, flow_port[key][i][1], 2048, flow_port[key][i][2], flow_port[key][i][3], flow_port[key][i][4], flow_port[key][i][5], 30, key[1], self.free_bandwidth[dpid][self.sw_out_inf[dpid]], self.redir_flow_num)
				else:
		...
		for stat in sorted(body, key=attrgetter('port_no')):
			...
			if port_no != ofproto_v1_3.OFPP_LOCAL:
				...
				if len(tmp) > 1: 
					...
					pre = tmp[-2][0]
					period = self._get_period(tmp[-1][3], tmp[-1][4], tmp[-2][3], tmp[-2][4])	#(now_duration_sec+now_duration_nsec)-(pre_duration_sec+pre_duration_nsec)
				speed = self._get_speed(self.port_stats[key][-1][0], pre, period)	#(now_tx_bytes-pre_tx_bytes)/period 
				self._save_stats(self.port_speed, key, speed, 5)
				self._save_freebandwidth(dpid, port_no, speed)
		
		#update polling interval
		for i in xrange(3001,3009):	#get edge switch utilization
	...
	def install_flow(self, datapaths, link_to_port, path, flow_info):
		...
		for i in xrange(1, len(path) - 1):
			...
			if port and port_next:
				...
				datapath = datapaths[path[i]]
                                print("install flow 1",datapath,flow_info,src_port,dst_port)
		...
		out_port = port_pair[0]
                print("install flow 2",first_dp,flow_info,in_port,out_port)
